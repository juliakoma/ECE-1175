...
int main() {
    ...
    float utilizationA = (float)execTimeA / periodA;
    float utilizationB = (float)execTimeB / periodB;
    cpuUtil = utilizationA + utilizationB;
    ...
    for (T = 0; T <= 200; T++) {
        ...
        if ((doA || doB) && T > absDeadlineA && T > absDeadlineB){
            printf("Deadline missed at T=%d, exiting...\n", T);
            break;
        }
        // End of your code
        ...
        if (T % periodA == 0 && T % periodB != 0) {
            ...
            if (tB < execTimeB) { // process B is unfinished yet
                ...
                if (doB && absDeadlineB - T < absDeadlineA - T)
                {
                    printf("When T=%d, preempted process B%d to run A%d\n", T, jB, jA);
                    doA = 1;
                    doB = 0;
                }
                // End of your code
        ...
        if (T % periodA != 0 && T % periodB == 0) {
            ...
            if (tA < execTimeA) { // process A is unfinished yet
                // Your code here to handle preemption
                // Please print out the decision
                if (doB && absDeadlineA - T < absDeadlineB - T)
                {
                    printf("When T=%d, preempted process A%d to run B%d\n", T, jA, jB);
                    doA = 0;
                    doB = 1;
                }
                // End of your code
